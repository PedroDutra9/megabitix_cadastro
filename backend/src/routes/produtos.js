...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (err.code === '23505') return res.status(409).json({ error: 'Número de série já cadastrado: ' + err.detail });
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await client.query(
      'UPDATE produtos SET nome=$1, categoria=$2, marca=$3, modelo=$4, descricao=$5 WHERE id=$6 RETURNING *',
      [nome, categoria, marca, modelo, descricao, req.params.id]
    );
    if (!rows.length) { await client.query('ROLLBACK'); return res.status(404).json({ error: 'Produto não encontrado' }); }

    // Adiciona séries novas (não remove existentes)
    for (const sn of seriais) {
      await client.query(
...
// DELETE /api/produtos/serial/:serialId
router.delete('/serial/:serialId', async (req, res) => {
  try {
    const { rows } = await query('SELECT status FROM seriais WHERE id = $1', [req.params.serialId]);
    if (!rows.length) return res.status(404).json({ error: 'Série não encontrada' });
    if (rows[0].status === 'vendido') return res.status(409).json({ error: 'Série já vendida, não pode ser removida' });
...
// GET /api/produtos/estoque/resumo
